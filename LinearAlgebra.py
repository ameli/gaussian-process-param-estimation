...
def LanczosTridiagonalization(A,w,m):
    """
    Tridiagonalizing matrix A to T using the start vector w. m is the Lanczos
    degree, which will be the size of square matric T.
...
def LanczosTridiagonalization2(A,v,m):
    """
    Algorithm is based "Templates for solution of Algebrai Eigenvalue Problems", James Demmel, p.57
    """
...
def GolubKahnLanczosBidiagonalization(A,w,m):
    """
    Inputs:
    ...
    Reference:
        -Algorithm 6.27 in http://www.netlib.org/utk/people/JackDongarra/etemplates/node198.html
        -Matrix Computations, Golub, p. 495
        -Templates for Solution of Algebraic Eigenvalue Problem, Demmel, p. 143

    Issues:
        When matrix A is very close to the identity matrix, the GK bidoagonalization method can not 
        find beta, as beta becomes zero. If A is not exactly identity, you may descrease the Tolerance
